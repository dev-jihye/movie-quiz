...
import { showQuiz } from "../__generated__/showQuiz";
...
export default function ShowQuiz() {
  ...
  const { loading, data, refetch } = useQuery<showQuiz>(SHOW_QUIZ_QUERY, {
    variables: {
  ...
  useEffect(() => {
    if (data?.showQuiz) {
      if (data?.showQuiz?.isWinner) {
        ...
        //문제를 맞춘 적이 없거나 틀렸을 때
        if (data?.showQuiz?.nextTry && data?.showQuiz?.nextTry > 0) {
          setIsAnswer(true);
